...
### DoAsync(com, method)
Метод запускает асинхронную задачу без параметров, возвращает 0 или -1 в случае если задача не создана. Первым параметром передается открытый COM-объект, вторым параметром — текстовое наименование метода.
### DoAsync1(com, method, val1)
Метод запускает асинхронную задачу с одним требуемым параметром val1, возвращает 0 или -1 в случае если задача не создана. Первым параметром передается открытый COM-объект, вторым параметром — текстовое наименование метода, третьим — значение параметра, передаваемое указанному методу.
### DoAsync2(com, method, val1, val2)
Метод запускает асинхронную задачу с двумя требуемыми параметрами val1 и val2, возвращает 0 или -1 в случае если задача не создана. Первым параметром передается открытый COM-объект, вторым параметром — текстовое наименование метода, третьим и четвертым — значения передаваемые указанному методу.
### DoAsync3(com, method, val1, val2, val3)
Метод запускает асинхронную задачу с тремя требуемыми параметрами val1, val2 и val3, возвращает 0 или -1 в случае если задача не создана. Первым параметром передается открытый COM-объект, вторым параметром — текстовое наименование метода. Третьим, четвертым и пятым передаются значения  параметров для указанного метода.
...
### ITask_OnError(err)
Метод обеспечивает обратный вызов через интефейс ITask и событие OnError. Параметром является строка, содержащая текст описания ошибки, возникшей в асинхронной задаче. Если обнаруживается этот метод на вызывающей стороне, то он получает управление при возникновении ошибки. Смотрите ниже приведенный пример на языке Visual FoxPro.  
### ITask_OnEnded(ret)
...
Примечание. VFPA версий 10 и 10.1 не поддерживает работу интерфесов с обратными вызовами.  
...
0.1.2.0. 09.04.2025. Добавлен обратный вызов при аозникновении ошибки в асинхронной задаче.  
