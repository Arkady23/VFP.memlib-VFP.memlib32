...
```
### WriteUtil(input)
Метод помещает стрку input, заданную параметром, в стандартный ввод утилиты, если та ожидает его. Далее входной поток закрывается и утилита начинает обработку. Пример с использованием утилыты [gawk.exe](https://github.com/Arkady23/gawk.exe):
```xBase
oMem = CreateObject('VFP.memlib32')
