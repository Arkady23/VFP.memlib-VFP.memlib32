...
### ITask_OnError(err)
Метод обеспечивает обратный вызов через интефейс ITask и событие OnError. Параметром является строка, содержащая текст описания ошибки, возникшей в асинхронной задаче. Если обнаруживается этот метод на вызывающей стороне, то он получает управление при возникновении ошибки. Смотрите ниже пример 3 на языке Visual FoxPro.  
