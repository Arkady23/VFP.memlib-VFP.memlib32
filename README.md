...
Метод запускает утилиту без параметров или, если неоходимо, со строкой аргументов. Метод может реализовывать цепочку команд, методом трубы, скрепленных символом "|". При этом в параметр аргументов также передаются параметры скрепленные символом "|". Если количество групп параметров не совпадает с количеством утилит, то утилиты или параметры дополняются последнией указанной утилитой или последней указанной группой параметров. Последним параметром указывается номер кодовой страницы текста, принимаемого и передаваемого утилитами. Метод возвращает логическое значение правда, если все указанные параметром утилиты обнаружены. Пример:
```xBase
