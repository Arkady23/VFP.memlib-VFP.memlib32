...
```
Если до выполнения метода Write() был сформирован указатель чтения при использовании методов Read(), ReadLine() или ReadToEnd(), то после исполнения Write() указатель чтения сбрасывается в начало потока Stream. Эту особенность можно использовать для сброса указателя чтения командой `oMem.write("")`.
### lenStream()
